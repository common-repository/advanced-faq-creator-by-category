...
class AFAQCBC_collapse_2_col{
        ...
        if($searchVisibility == 1){
            ...
            foreach ($questions as $cat) {
				...
				if(get_option('faq_cat_page') && get_option('faq_cat_page') != -1 ){
					$category_link = esc_url(get_page_link(get_option('faq_cat_page')).'?faq_id='.$faq_id.'&cat_id='.$cat_id);
				}else{
					$category_link = '';
				}
    ...
    public function load_script_css(){
		...
		wp_enqueue_style('font-awesome-collapse', AFAQCBC_PLUGIN_URL.'/assets/css/font-awesome.min.css');
